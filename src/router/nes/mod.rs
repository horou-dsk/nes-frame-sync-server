...
#[get("/join_room")]
pub async fn join_room(info: web::Query<HashMap<String, String>>, srv: web::Data<Addr<server::OnLineServer>>) -> HttpResultResp {
    let room = info.0["room"].clone();
    let room = room.parse::<u16>().unwrap();
    info!("room = {}", room);
    let room_info = srv.get_ref().send(GetRoomInfo(room)).await.expect("join room error!");
    info!("{:?}", room_info);
